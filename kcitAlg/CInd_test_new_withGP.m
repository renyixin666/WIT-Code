function [Sta, Cri, p_val, Cri_appr, p_appr,ind] = CInd_test_new_withGP(x, y, z, alpha, width)
...
IF_GP = 1;
Approximate = 1;
Bootstrap = 0; % Note: set to 0 to save time if you do not use simulation to generate the null !!!
...
%     fprintf('Optimization hyperparameters in GP regression:\n');
    ...
    [logtheta_x, fvals_x, iter_x] = minimize(logtheta0, 'gpr_multi', -500, covfunc, z, 2*sqrt(T) *eix * diag(sqrt(eig_Kx))/sqrt(eig_Kx(1)));
    [logtheta_y, fvals_y, iter_y] = minimize(logtheta0, 'gpr_multi', -500, covfunc, z, 2*sqrt(T) *eiy * diag(sqrt(eig_Ky))/sqrt(eig_Ky(1)));
    covfunc_z = {'covSEard'};
